...
GRectangle::GRectangle(glm::mat4 VP_Mat) : viewproj_mat(VP_Mat),
...
void GRectangle::UpdatePosition(glm::vec2 pos)
{
	this->pos.x = pos.x;
	this->pos.y = pos.y;
}

void GRectangle::UpdateRotation(float angle)
{
	this->angle = angle;
}

void GRectangle::UpdateScale(glm::vec2 scale)
{
	this->scale.x = scale.x;
	this->scale.y = scale.y;
}

glm::vec2 GRectangle::getPosition()
{
	glm::vec2 res;
	res.x = pos.x;
	res.y = pos.y;
...
glm::vec2 GRectangle::getrectSize()
{
	glm::vec2 res;
	res.x = scale.x;
	res.y = scale.y;
